...
function App() {
  ...
  useEffect(() => {
    ...
    if (window.electronAPI) {
      window.electronAPI.onActivityUpdate((_, stats) => {
    ...
    <div style={{ width: '100%', maxWidth: '100%', overflowX: 'hidden' }}>
      ...
      {!isLoggedIn ? (
        ...
        <>
          ...
          {/* Project Select with containment */}
                ...
                >
                  {[t, ...allTasks.filter(st => st.parent_id === t.id)].map(st => (
                    <option
                      key={st.id}
                      value={st.id}
                      title={st.name} // Hover tooltip shows full name
                    >
          ...
          <div style={{ fontSize: '11px', textAlign: 'center', color: '#555' }}>
            Mouse: {activityStats.mouse} | Keyboard: {activityStats.keyboard}
          </div>
        </>
      )}
    </div>
